...
Example 1: Static binary logit (Angrist and Evans 1998)
---------------------------------------------------
...
Example 2: Static logit with a model-specific prior (MCMC) + convergence diagnostics
----------------------------------------------------------------------------------

This example illustrates two features:

1. Using a model-specific bias-reducing prior (tailored for static exponential-family models).
...
.. code-block:: text
   ...
   ---- ESTIMATION RESULTS --------------------------------------------------------
                    LOGIT PANEL MODEL WITH TWO-WAY FIXED EFFECTS
       USING BIAS-REDUCING PRIOR TAILORED FOR STATIC EXPONENTIAL FAMILY MODELS
   --------------------------------------------------------------------------------
   Number of Individuals:   664                              Observations:     5976
...
.. figure:: ../_static/logit_mcmc_trace.png
   :alt: MCMC trace/hist/ACF for beta_1 (static logit)
   :align: center
   :width: 90%
...
.. figure:: ../_static/logit_mcmc_hist.png
   :alt: MCMC trace/hist/ACF for beta_2 (static logit)
   :align: center
   :width: 90%
...
.. figure:: ../_static/logit_mcmc_acf.png
   :alt: MCMC trace/hist/ACF for beta_3 (static logit)
   :align: center
   :width: 90%

   MCMC diagnostics for :math:`\\beta_3`.

.. figure:: ../_static/logit_mcmc_geweke.png
   :alt: MCMC trace/hist/ACF for beta_4 (static logit)
   :align: center
   :width: 90%

   MCMC diagnostics for :math:`\\beta_4`. (In this run the ACF decays slowly, suggesting
   stronger persistence and the potential value of longer chains / more thinning if desired.)
...
.. code-block:: text
   ...
   --------------------------------------------------------------------------------
   Note: Prior is constructed based on the heteroskedasticity-autocorrelation con-
         sistent (HAC) estimation with the Bartlett kernel and lag of 1.
   Note: Panel data contains under-identified observations;
         Dropped 797 out of 1461 individuals, and 0 out of 9 time periods. 
...
- ``lag=1`` controls the truncation level in the HAC-style approximation used when constructing
  spectral-expectation objects entering bias correction and (when applicable) analytical steps.
...
tailored for dynamic multinomial logit (``prior="DML"``).
...
.. code-block:: python
   ...
   # Bias-reducing prior for dynamic multinomial logit (HAC truncation uses lag=1 here)
   res_mlogit_dml = twowaypanel.fit(
       Y, X,
       model="mlogit", prior="DML", lag=1,
       algorithm="JML"
   )
...
.. code-block:: text
   ...
   ---- ESTIMATION RESULTS --------------------------------------------------------
                MULTINOMIAL LOGIT PANEL MODEL WITH TWO-WAY FIXED EFFECTS
   USING BIAS-REDUCING PRIOR TAILORED FOR DYNAMIC EXPONENTIAL FAMILY MODELS
   --------------------------------------------------------------------------------
   Number of Individuals:    44                              Observations:      660
...
- The DML prior changes both point estimates and the fitted objective, reflecting the role of
  likelihood-based bias correction in finite samples.
- The note about HAC/Bartlett/lag explains how the package approximates spectral expectations
  entering the correction terms in dynamic settings.

...
- Common choices (following Yan et al., 2026) include:
  ...
  - ``prior="SE"``      : tailored for **static** binary logit
  - ``prior="DE"``      : tailored for **dynamic** binary logit
  - ``prior="SML"``     : tailored for **static** multinomial logit
  - ``prior="DML"``     : tailored for **dynamic** multinomial logit
...
- In the dynamic logit setting, if ``prior="SE"`` and ``ac=True``, the workflow follows Yan et al. (2026): the
...
Serial dependence / truncation: ``lag``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

For dynamic models, several correction terms involve **spectral expectations**. In practice, these objects are approximated using a **truncated lag window** (a HAC-style truncation). The argument ``lag`` controls the truncation level.
...
As a rule of thumb, ``lag`` should be chosen in line with the persistence in the data and the dynamic specification;
